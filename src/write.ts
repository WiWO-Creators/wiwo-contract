...
export const WRITE_AUTH_SCHEME = 'Bearer';
 ...
 */
export type WiwoArticleDraft = Omit<
  WiwoArticle,
  'id' | 'url' | 'updatedAt'
> & { id?: string };
 ...
 */
export function parseArticleDraft(value: unknown): WiwoArticleDraft | null {
  ...
  if (!esMarkdown && !esBloques) return null;

  if (value.id !== undefined && typeof value.id !== 'string') return null;

  return value as unknown as WiwoArticleDraft;
}
 ...
 */
const EN_EL_NUCLEO: Record<string, (draft: WiwoArticleDraft) => unknown> = {
  title: (d) => d.title,
  summary: (d) => d.summary,
  tags: (d) => d.tags,
  ...
  body: (d) =>
    d.body?.format === 'markdown' ? d.body.markdown : d.body?.blocks,
};

/** Las claves del manifest que el núcleo del contrato cubre. */
export const CORE_FIELD_KEYS = Object.keys(EN_EL_NUCLEO);
 ...
 */
function readField(draft: WiwoArticleDraft, key: string): unknown {
  const enNucleo = EN_EL_NUCLEO[key];
  if (enNucleo) {
    const valor = enNucleo(draft);
    if (!estaVacio(valor)) return valor;
  }
...
export function validateAgainstFields(
  ...
  for (const field of fields) {
    ...
    if (estaVacio(valor)) {
      if (field.required) errores[field.key] = `Falta "${field.label}".`;
    ...
    if (field.key === 'body') {
      const esperado =
        field.type === 'blocks' ? 'blocks' : field.type === 'markdown' ? 'markdown' : null;
    ...
    if (field.type === 'enum') {
      ...
      if (permitidos.length > 0 && !permitidos.includes(valor as string)) {
