...
export function createMediaStore(
  ...
  return {
     ...
     * nunca se guardó.
     */
    async save(file: WiwoMediaFile): Promise<void> {
      const sql = await getSql();
      ...
      await sql.query(
        `insert into ${WIWO_MEDIA_TABLE} (id, content_type, bytes, data)
         values ($1, $2, $3, $4)
         on conflict (id) do nothing`,
        [file.id, file.contentType, file.bytes, Buffer.from(file.data)],
      );
    },

    async find(id: string): Promise<WiwoStoredMedia | null> {
      const sql = await getSql();
      const filas = await sql.query<{ content_type: string; data: unknown }>(
        `select content_type, data from ${WIWO_MEDIA_TABLE} where id = $1`,
        [id],
      );
 ...
 */
export function canWriteMedia(config: WiwoSiteConfig): boolean {
 ...
 */
export function createMediaHandlers(config: WiwoSiteConfig): {
  ...
  return {
    async POST({ request }) {
      ...
      if ('code' in archivo) return writeError(archivo);
 ...
 * nunca nada.
 */
export function createMediaFileHandlers(config: WiwoSiteConfig): {
  GET(ctx: { request: Request; params: { id: string } }): Promise<Response>;
} {
  return {
    async GET({ params }) {
      if (!config.media) return errorResponse('Este sitio no sirve archivos.', 404);
      ...
      if (!archivo) return errorResponse('Ese archivo no existe.', 404);

      return new Response(archivo.data as BodyInit, {
        headers: {
