...
export const WIWO_MEDIA_TABLE = 'wiwo_media';
 ...
 */
export const WIWO_MEDIA_TYPES: Readonly<Record<string, string>> = {
  'image/jpeg': 'jpg',
  'image/png': 'png',
  'image/webp': 'webp',
  'image/avif': 'avif',
 ...
 */
export const WIWO_MEDIA_MAX_BYTES = 5 * 1024 * 1024;
...
export async function mediaId(
  ...
  const hex = Array.from(new Uint8Array(digest))
    ...
    .join('');
  return `${hex.slice(0, 32)}.${WIWO_MEDIA_TYPES[contentType]}`;
}

/** El tipo de contenido de un archivo guardado, por su extensión. */
export function mediaContentType(id: string): string | null {
  const extension = id.slice(id.lastIndexOf('.') + 1);
  const encontrado = Object.entries(WIWO_MEDIA_TYPES).find(
    ([, propia]) => propia === extension,
  );
...
export async function parseMediaUpload(
  request: Request,
): Promise<WiwoMediaFile | WiwoWriteError> {
  const contentType = (request.headers.get('content-type') ?? '')
    .split(';')[0]
    .trim()
    .toLowerCase();

  if (!WIWO_MEDIA_TYPES[contentType]) {
    return {
      code: 'validation',
      message: `Este sitio acepta ${Object.keys(WIWO_MEDIA_TYPES).join(', ')}; llegó "${contentType || 'nada'}".`,
    };
  }
  ...
  if (Number.isFinite(declarado) && declarado > WIWO_MEDIA_MAX_BYTES) {
  ...
  if (data.byteLength > WIWO_MEDIA_MAX_BYTES) {
...
function demasiadoGrande(bytes: number): WiwoWriteError {
  const mega = (valor: number) => `${(valor / (1024 * 1024)).toFixed(1)} MB`;
  return {
    code: 'validation',
    message: `El archivo pesa ${mega(bytes)} y el máximo es ${mega(WIWO_MEDIA_MAX_BYTES)}.`,
  };
}
