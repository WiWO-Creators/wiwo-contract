 ...
 *   servidor.
 */
function createArticleStore(getSql) {
    return {
        /**
        ...
        async read(since) {
            try {
                ...
                const filas = since
                    ? await sql.query(`select article from ${exports.WIWO_ARTICLES_TABLE} where updated_at >= $1 order by published_at desc, id asc`, [since])
                    : await sql.query(`select article from ${exports.WIWO_ARTICLES_TABLE} order by published_at desc, id asc`);
        ...
        async find(id) {
            const sql = await getSql();
            const filas = await sql.query(`select article from ${exports.WIWO_ARTICLES_TABLE} where id = $1`, [id]);
        ...
        async save(article) {
            const sql = await getSql();
            await sql.query(`insert into ${exports.WIWO_ARTICLES_TABLE} (id, published_at, updated_at, article)
         values ($1, $2, $3, $4)
        ...
        async remove(id) {
            const sql = await getSql();
            const filas = await sql.query(`delete from ${exports.WIWO_ARTICLES_TABLE} where id = $1 returning id`, [id]);
 ...
 * después.
 */
function mergeArticles(archivo, publicadas) {
...
function createArticlesHandlers(config) {
    return {
        async GET({ request }) {
            ...
            if (since !== null && !(0, http_js_1.isIsoDate)(since)) {
                return (0, http_js_1.errorResponse)(`El parámetro "since" debe ser una fecha AAAA-MM-DD; llegó "${since}"`, 400);
            }
            const origin = (0, http_js_1.publicOrigin)(request);
            const todas = await allArticles(config, origin, since ?? undefined);
            const { items, nextCursor } = (0, pagination_js_1.sliceByCursor)(todas, (article) => article.id, (0, pagination_js_1.resolvePageSize)(params.get('limit')), params.get('cursor'));
            return (0, http_js_1.jsonResponse)({
                articles: items,
                count: items.length,
                nextCursor,
        ...
        async POST({ request }) {
            ...
            const draft = (0, write_js_1.parseArticleDraft)(cuerpo);
            if (!draft) {
                return writeError({
                    code: 'validation',
            ...
            const origin = (0, http_js_1.publicOrigin)(request);
            const errores = (0, write_js_1.validateAgainstFields)(draft, config.fields(origin));
        ...
        async DELETE({ request }) {
            ...
            if (!(await config.store.remove(id))) {
                return writeError({
                    code: 'not_found',
                    message: 'Este sitio no tiene publicada ninguna nota con ese identificador. Si aparece en el sitio, es parte de su archivo editorial y vive en su repositorio, no en la base: no se puede borrar por API.',
                });
            }
            return (0, http_js_1.jsonResponse)({ id, deleted: true }, 200);
        },
    };
}
