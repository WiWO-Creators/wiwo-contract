...
exports.sliceByCursor = sliceByCursor;
...
function resolvePageSize(raw) {
    ...
    if (!Number.isFinite(pedido) || pedido < 1)
        return exports.DEFAULT_PAGE_SIZE;
    return Math.min(Math.trunc(pedido), exports.MAX_PAGE_SIZE);
}
/**
...
function encodeCursor(id) {
    return node_buffer_1.Buffer.from(id, 'utf8').toString('base64url');
}
/**
...
function decodeCursor(cursor) {
    if (!/^[A-Za-z0-9_-]+$/.test(cursor))
        return null;
    const id = node_buffer_1.Buffer.from(cursor, 'base64url').toString('utf8');
    if (id.length === 0)
        return null;
 ...
 */
function sliceByCursor(sorted, idOf, pageSize, cursor) {
    let desde = 0;
    if (cursor) {
